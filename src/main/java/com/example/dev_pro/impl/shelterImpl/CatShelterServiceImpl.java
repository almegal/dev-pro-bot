...
public class CatShelterServiceImpl implements CatShelterService {
    ...
    public void handleUpdate (Update update) {
        ...
        switch (text) {
            case INFO_COM:
                sendMessage = new SendMessage(chatId, tBotConfig.getInfoMsgCatShelter());
                sendMessage.replyMarkup(getInfoKeyboardButtons());
                telegramBot.execute(sendMessage);
                break;
            case OVERVIEW_COM:
                sendMessage = new SendMessage(chatId,tBotConfig.getOverviewMsgCatShelter());
                telegramBot.execute(sendMessage);
                break;
            case ADDRESS_COM:
                sendMessage = new SendMessage(chatId,tBotConfig.getDirectionsMsgCatShelter());
                listener.sendPhoto(chatId, tBotConfig.getDirectionsMsgCatShelter(),
                        "static/images/driving_directions_cat.jpg");
                telegramBot.execute(sendMessage);
                break;
            case CAR_PASS_COM:
                sendMessage = new SendMessage(chatId,tBotConfig.getCarPassMsgCatShelter());
                telegramBot.execute(sendMessage);
                break;
            case SAFETY_RULES_COM:
                sendMessage = new SendMessage(chatId,tBotConfig.getSafetyRulesMsgCatShelter());
                telegramBot.execute(sendMessage);
                break;
            case USER_CONTACT_COM:
                sendMessage = new SendMessage(chatId,tBotConfig.getUserContactMsgCatShelter());
                telegramBot.execute(sendMessage);
                break;
            case COME_BACK_COM:
                sendMessage = new SendMessage(chatId, tBotConfig.getComeBackMsgCatShelter());
